...
export function AdminSidebar({ operator }: { operator: string }) {
  ...
  return (
    <aside className={styles.sidebar} aria-label="Admin navigation">
      <Link className={styles.brand} href="/admin">EcoGifts Admin</Link>
      <p className={styles.eyebrow}>Operations</p>
      <nav>{items.map((item) => <NavItem item={item} path={path} key={item.href} />)}</nav>
      <div className={styles.account}>
        <span>{operator} · Admin</span>
        <button disabled={pending} onClick={logout} type="button">{pending ? "Signing out…" : "Sign out"}</button>
      </div>
    </aside>
  );
}

function NavItem({ item, path }: { item: typeof items[number]; path: string }) {
  const active = item.href === "/admin" ? path === item.href : path.startsWith(item.href);
  if (!item.enabled) return <span aria-disabled="true" className={styles.navDisabled}>{item.label}</span>;
  return <Link aria-current={active ? "page" : undefined} className={active ? styles.navActive : styles.navLink} href={item.href}>{item.label}</Link>;
}
